<script>
  ...
  import PartnerCart from './PartnerCart.svelte';
...
<div class="floating-accordion-container">
  ...
  {#if $isCartOpen}
    ...
    >
      <PartnerCart on:close={() => $isCartOpen = false} on:orderCreated={() => $isCartOpen = false} />
    </div>
  {/if}
  ...
  <div class="toggle-bar shadow-2xl border-2 border-indigo-500/30">
    ...
    >
      ...
      <div class="flex-1 text-left">
        <span class="text-xs font-black uppercase opacity-60 tracking-widest block leading-none mb-1">Tu Carrito</span>
...
<style>
  ...
  .accordion-panel {
    background-color: #0f172a; /* Solid Navy Dark */
    border-radius: 2rem;
    height: 600px;
